...
class DataProcessor:
    ...
    def save_artifacts(self):
        try:
            ...
            try:
                joblib.dump(self.y_test,    Y_TEST)
                logger.info(f"y_test saved at {Y_TEST}")
            except Exception as e:
                logger.error(f"Error dumping y_test ({Y_TEST}): {e!r}")
                raise

            self.rating_df.to_csv(RATING_DF, index=False)
            logger.info("All the training/testing data and rating_df are saved now.")
        except Exception as e:
            logger.error(f"Error saving artifacts: {e!r}")
            raise CustomException(f"Failed to save artifacts data: {e}", sys)
