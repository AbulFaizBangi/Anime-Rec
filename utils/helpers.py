...
def getSynopsis(anime, path_synopsis_df):
    try:
        ...
        if isinstance(anime, int):
            ...
            raise ValueError("Anime must be int (ID) or str (name).")
        
        if result.empty:
            raise ValueError(f"No synopsis found for {anime}")

        # Use the correctly spelled column name - 'sypnopsis' instead of 'synopsis'
        return result.sypnopsis.values[0]
    except Exception as e:
        print(f"Error in getSynopsis: {e}")
